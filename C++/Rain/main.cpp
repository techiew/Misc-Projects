...
/*
Welcome to my simple and inefficient rain simulation
Made with SDL2 (creates the window, manages input, etc.) and SDL2_gfx (drawing circles and other shapes) 
	...
	while (isRunning) {
		...
		if (SDL_PollEvent(&event)) {
			...
			if (event.type == SDL_MOUSEMOTION) {
				...
				if (isMousePressed) {
					//If the mouse is not colliding with the previously created obstacle, create a obstacle
