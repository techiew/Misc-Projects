...
Player::Player(Map* map, Camera2D* camera, int playerSize, int maxHP, double movementPerTick, bool godmode) {
	this->map = map;
	this->camera = camera;
	this->playerSize = playerSize;
	this->pos = map->spawnpoint;
...
void Player::SetAimPos(Vector2 mousePos) {
	...
	Vector2 mouseAngle(mousePos.x - (pos.x - camera->offset.x), mousePos.y - (pos.y - camera->offset.y));
